# filters_cd.py
# Continuous–discrete filters with stiff ODE time-updates.
...
# Φ(·) operator (kept for potential square-root ODE forms)
...
class CDEKF:
    ...
    def update(self, x_pred: np.ndarray, P_pred: np.ndarray, z: np.ndarray):
        ...
        # P H^T S^{-1} using solve (more stable than inv)
        K = np.linalg.solve(S, (H @ P_pred.T)).T
        x_upd = x_pred + K @ y
        P_upd = ensure_psd(P_pred - K @ S @ K.T)
        return x_upd, P_upd, y, S

# --------------------------- CD-CKF (measurement SR) ------------------------

class CDCKF:
    """CD-CKF using matrix MDE for time update and cubature at measurement."""
    ...
    @staticmethod
    def _cubature_points(x: np.ndarray, P: np.ndarray):
        n = x.size
    ...
    def update(self, x_pred: np.ndarray, P_pred: np.ndarray, z: np.ndarray):
        Xi, W = self._cubature_points(x_pred, P_pred)
...
# --------------------------- CD-UKF (paper weights) -------------------------

class CDUKF:
    """CD-UKF using matrix MDE for time update and the paper’s sigma-point
    weights at the measurement step (no alpha/kappa)."""
    def __init__(self, cm: ContinuousModel, h_fun, R,
                 rtol: float = 1e-12, atol: float = 1e-12, max_step: float = 1e-1):
        self.cm = cm
        self.h  = h_fun
    ...
    @staticmethod
    def _sigma_points_paper(x: np.ndarray, P: np.ndarray):
        n = x.size
        S = safe_cholesky(ensure_psd(P))
        Xi = np.zeros((2*n + 1, n))
        Xi[0] = x
    ...
    def update(self, x_pred: np.ndarray, P_pred: np.ndarray, z: np.ndarray):
        Xi, wm = self._sigma_points_paper(x_pred, P_pred)
        Zsig = np.array([self.h(xi) for xi in Xi])
        ...
        for i in range(Xi.shape[0]):
            dz = (Zsig[i] - z_pred).reshape(-1, 1)
            dx = (Xi[i]   - x_pred).reshape(-1, 1)
            wi = (1.0/6.0) if i > 0 else (3.0 - x_pred.size) / 3.0
            S  += wi * (dz @ dz.T)
            Pxz += wi * (dx @ dz.T)
        S = ensure_psd(S)
        K = np.linalg.solve(S, Pxz.T).T
